...
export async function POST(request: Request) {
  try {
    ...
    if (!body.nombre || !body.email || !body.numero || !body.dni || !body.vehiculo) {
      return NextResponse.json(
        { error: 'Faltan campos requeridos: nombre, email, teléfono, DNI y vehículo son obligatorios.' },
        { status: 400 }
      );
    }
    ...
    const rowData = [
      ...
      body.mensaje || '',
      body.vehiculo,
      `S/ ${body.precio.toFixed(2)}`
    ];

    // Añadir nueva fila a la hoja de cálculo
    const response = await sheets.spreadsheets.values.append({
      spreadsheetId: process.env.GOOGLE_SHEET_ID,
      range: 'A1:H1', // Rango para autodetectar
